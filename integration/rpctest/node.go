...
import (
	...
	rpc "github.com/coinsuite/coind/rpcclient"
)

// nodeConfig contains all the args, and data required to launch a coind process
...
func newConfig(prefix, certFile, keyFile string, extra []string) (*nodeConfig, error) {
	coindPath, err := coindExecutablePath()
	if err != nil {
		coindPath = "coind"
	}
...
// arguments returns an array of arguments that be used to launch the coind
// process.
...
// command returns the exec.Cmd which will be used to start the coind process.
...
// to the coind process that is launched via Start().
...
// as the base for the log and data directories for coind.
...
// start creates a new coind process, and writes its pid in a file reserved for
...
// stop interrupts the running coind process process, and waits until it exits
...
// shutdown terminates the running coind process, and cleans up all
